...
### The shared required-input runtime

Milestone 6.1 extracted the first shared producer-runtime primitive, and it is the only one extracted. `tools/producer/lib/required-inputs.mjs` decides, from a declaration alone, which outputs a missing required input withdraws and which carry on without it. It implements STD-0011 R-53 and reads the shape STD-0010 R-48 fixes; the disclosure it puts on a degraded output is the one STD-0011 R-25 requires. It holds no artifact type identity, no methodology name, and no knowledge of stage ordering, evidence, records, or the subject, and a test asserts that by reading its own source.

Both consuming producers now use it, and both declare `consumes` in the R-48 shape rather than holding an inverted constant. The direction changed with the shape: `REQUIRED_INPUTS` mapped an output to the inputs it needed, and R-48 maps an input to the outputs that require it, which is the direction a consumer reads. AUD-0002 was left alone: it consumes nothing, so it has no required input to gate, and giving it the primitive for symmetry would have been abstraction without a subject.
