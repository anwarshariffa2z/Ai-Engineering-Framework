...
| A generic producer runtime | **Answered, and the answer is not yet.** Three producers now occupy the three positions the question needs: one that consumes nothing, one that consumes from a single methodology, and one that consumes from two. Three of five extraction conditions hold — an identical gate-and-write lifecycle, identical identity and resolution mechanics, and stage ordering derived from what each methodology declares it consumes. Two fail, and they are one thing seen twice: failure propagation differs by methodology, and the mapping from a producer's outputs to the inputs each output requires is methodology-specific and derivable from no declaration | A declarative per-output required-input contract. The framework can state what a producer consumes and cannot state which of its outputs cannot be produced without which input, so any shared runtime would carry a per-methodology table and hide exactly what differs. That contract is the next design gate, and extraction is decided after it rather than around it |
...
| No rule classifies a document version change | [STD-0010](../02-methodology/metadata-specification.md) R-05 states a version's form and nothing about what changes MAJOR, MINOR, or PATCH for a document. [STD-0013](../02-methodology/artifact-type-declaration-standard.md) R-15 and R-24 classify *artifact type* versions, which is a different subject. Three milestones have now bumped standard versions on precedent alone — additive requirement to MINOR — and precedent is not a rule | A standards decision on whether document version impact is classified, and if so where. It is a gap in the same family as `derives_from` versioning: a change is being made correctly and repeatedly, and nothing states that it is correct |
...
| --- | --- |
| Generic producer runtime extraction | Ordering is now derived from what each methodology declares it consumes rather than hardcoded, and the gate-and-write steps are identical across all three. What differs is failure propagation, which no two of the three exercise the same way |
| Empty-set aggregation | A third independent implementation reached the same lattice-bottom reading, for an artifact that is `NotApplicable` for a reason neither earlier subject produced |
...
| The fourth reference producer | Chosen on information gain, as this one was |
