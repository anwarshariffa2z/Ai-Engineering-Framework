...
| Shared Producer Runtime | **First primitive delivered, and only the first.** Milestone 6.1 extracted required-input gating and degradation into `tools/producer/lib/required-inputs.mjs`, implementing [STD-0011](../02-methodology/contract-specification.md) R-53 against the shape [STD-0010](../02-methodology/metadata-specification.md) R-48 fixes. Both consuming producers use it and both now declare `consumes` in that shape instead of holding an inverted constant; AUD-0002 consumes nothing and was left unchanged. Every artifact in all three runs is byte-identical and no digest moved. Classified **PROVEN**. Nothing further is extracted: run assembly, envelope construction, and artifact emission are reused already without an abstraction over them, and stage execution, record construction, and lineage construction remain methodology-specific |
| Methodology Producer Contracts | **Delivered.** The core architecture already assigns a Methodology responsibility for declaring the artifact types it produces and consumes. Milestone 6.2 implements the missing machine-readable representation that makes that existing responsibility validator-visible: [STD-0010](../02-methodology/metadata-specification.md) R-49 declares `producer_kinds` as the join to the type declarations, and `consumes` carries the members R-26, R-27, and R-48 already require. Representational completion within Framework v1, not a new object or an architectural redesign — no object type, no layer, no ADR, no identifier namespace. Nine checks bound, 130 to 139, of which [STD-0011](../02-methodology/contract-specification.md) R-21 compares the authored contract against emitted artifacts rather than against other documents. Three methodologies carry a consumption contract; the seven without a reference producer declare their kinds and wait, because R-48 admits no entry whose requiredness nothing states |
...
| Do the `-discovery` producer kinds mark a distinction or drift? | **Open, surfaced by the 6.2 join, deliberately not resolved.** Six artifact type declarations name a producer kind their siblings do not. `framework.feature.health` and `framework.feature.risks` declare `feature-discovery` where the other five feature types declare `feature-inventory`; `framework.gap.health` declares `gap-discovery` where the other five gap types declare `gap-analysis`; `framework.runtime.health` and `framework.runtime.risks` declare `runtime-discovery` where the other five runtime types declare `runtime-verification`. The pattern is exact and confined to health and risks types, which is what makes it ambiguous: a scoring artifact produced by a distinct stage is a defensible distinction, and so is a naming slip repeated three times. Nothing was normalized in 6.2 — AUD-0008, AUD-0010, and AUD-0011 truthfully claim both of their kinds, and the join is total and unambiguous either way | A reading of the three methodologies against their own stage definitions. If the kinds are one, six declarations are corrected and three methodologies drop a claim; if they are two, the distinction is stated where a reader can find it |
| Deriving a required-input contract from `derives_from` | **Closed, negatively, and it must not be attempted.** `derives_from` describes *provenance possibility*: records of this type may derive from that type. `required_for` describes *execution necessity*: this type cannot be produced without that input. Different relations, and no rule converts one into the other. Tested against both consuming producers, derivation fails in both directions. It over-derives: `framework.backend.resilience` derives from `framework.architecture.integrations` and does not require it, emitting `Partial` with records when that input is absent, so derivation would demand a sound artifact be withdrawn. It under-derives `consumes`: AUD-0003 consumes four types and derivation finds two, because `framework.architecture.scope` and `framework.architecture.integrations` are used for cross-checks that produce no derivation | Closed. The consumption contract is declared by the Methodology under R-49 and is never inferred |
| A generic producer runtime | **PARTIAL, and the named primitive is now extracted.** Milestone 6.1 delivered required-input gating as a shared module against R-53 and R-48, with both consuming producers migrated and every digest preserved. The verdict on the *whole* runtime is unchanged and is restated here rather than revisited. **Answered at the post-AUD-0005 gate: PARTIAL.** The design gate this entry named has been passed. [STD-0011](../02-methodology/contract-specification.md) R-53 states the per-output required-input contract and [STD-0010](../02-methodology/metadata-specification.md) R-48 represents it, so the mapping the framework could not express is now expressible. With it, the two conditions that failed collapse into one that holds: both consuming producers degrade by the same rule and differ only in the table, and a table that is a declaration is data rather than methodology logic. What does not follow is a whole-runtime extraction — stage ordering, subject knowledge, and record construction remain methodology-specific, and AUD-0002 exercises none of the consuming path | Extract the named primitive: required-input gating and degradation, driven by the R-48 declaration. Full extraction is not reconsidered until a producer exists that the primitive does not fit |
...
| Contract completion precedes producer implementation | **Standing rule, from 6.3 onward.** Each new reference producer completes its Methodology consumption contract before or as part of its implementation, so that the declaration and the implementation are authored against each other rather than one retrofitted to the other. AUD-0004 is the first producer built under this rule |
| An absent `consumes` is not an empty one | **Standing rule, normative at [STD-0010](../02-methodology/metadata-specification.md) R-50.** `consumes: []` is an established contract naming zero upstream types; an absent `consumes` key is a contract not yet established, and neither a reader nor a validator may treat the second as the first. For a Methodology with no reference producer, the absence of an operationalized `consumes` contract does not mean the methodology consumes nothing. Its prose identifies upstream artifact types; what it does not establish is requiredness, dependent outputs, understood compatibility, or a claimed profile, and [STD-0010](../02-methodology/metadata-specification.md) R-48 admits no entry without them. The contract is completed when the reference producer is implemented and those semantics have a second independent source to be tested against. A methodology that genuinely consumes nothing states so explicitly and is not represented by absence |
| The fourth reference producer | **Decided: AUD-0004 Frontend Discovery, as Milestone 6.3, on dependency rather than on information gain.** The sequence is 6.3 Frontend, 6.4 AUD-0006 Workflow, 6.5 AUD-0007 Security. Security is the higher-information target — eleven consumed types spanning five upstream methodologies, ten of them profiled, and the framework’s first cross-cutting consumer — and it is blocked, because `framework.frontend.exposure` and `framework.workflow.actors` are among its inputs and neither has a producer. Frontend and Workflow therefore precede Security, and each completes its own consumption contract when its producer is built |
